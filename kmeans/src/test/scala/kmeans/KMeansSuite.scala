...
class KMeansSuite extends FunSuite {
  ...
  test("'converged' should work for 'oldMeans' == GenSeq((1,1,1), ..., (99,99,99)) and 'newMeans' == GenSeq((1,1,1), ..., (99, 99, 99.01))") {
    val nn = 1 until 99
    val oldMeans = (nn, nn, nn).zipped.map { case (x,y,z) => new Point(x,y,z)}
    val newMeans = oldMeans.patch(98, Seq(new Point(99,99,99.01)), 1)
    assert(converged(0.01)(oldMeans, newMeans) == true)
  }
