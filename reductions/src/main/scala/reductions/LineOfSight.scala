...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    def go(pos: Int): Unit = {
      if (pos < output.length) {
        output(pos) = Math.max(output(pos - 1), input(pos - 1)/(pos -1))
        go(pos + 1)
      }
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    @annotation.tailrec
    def go(pos: Int, max: Float): Float =
      if (pos >= until) max
      else go(pos + 1, Math.max(max, input(pos)/pos))
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    @annotation.tailrec
    def go(pos: Int, max: Float): Unit =
      if (pos < until) {
        val current = Math.max(max, input(pos)/pos)
        output(pos) = current
        go(pos + 1, current)
      }

    go(from, startingAngle)
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = tree match {
      ...
      case Node(l, r) => parallel(downsweep(input, output, startingAngle, l), downsweep(input, output, Math.max(startingAngle, l.maxPrevious), r))
  }
